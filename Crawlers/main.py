...
class WebCrawler:
    ...
    def send_to_backend(self, url, content, content_type):
        try:
            backend_url = 'http://localhost:8080/api/contents'
            data = {
                'url': url,
                'content': content,
            ...
            if response.status_code == 200:
                ...
                print("Failed to send data to backend:", response.text)
    ...
    def on_message_received(self, ch, method, properties, body):
        ...
        content, content_type = self.crawl(url, depth=5)
        self.send_to_backend(url, content)  # Adjust the call to match the method signature

    def process_url(self, url):
        if url.lower().endswith(('.html', '.htm')):
            return 'txt', self.extract_text_from_html(url)
        elif url.lower().endswith(('.jpg', '.jpeg', '.png', '.gif')):
            return 'txt', self.process_image(url)
        elif url.lower().endswith('.docx'):
            return 'txt', self.process_word(url)
        elif url.lower().endswith('.csv'):
            return 'txt', self.process_csv(url)
        elif url.lower().endswith('.pdf'):
            return 'txt', self.process_pdf(url)
        elif url.lower().endswith(('.xls', '.xlsx')):
            return 'txt', self.process_excel(url)
        else:
            return 'other', "Unsupported content type"

    def crawl(self, url, depth=7):
        if depth == 0:
            return
